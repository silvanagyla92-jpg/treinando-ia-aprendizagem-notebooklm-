# Perguntas estratégicas e correlação com fontes

## Objetivo

Este documento reúne as cinco perguntas estratégicas do projeto e registra a correlação temática com as fontes do inventário de 25 documentos.

A correlação indica aderência temática entre pergunta e fonte. Ela não deve ser interpretada, isoladamente, como prova histórica de que uma fonte específica foi carregada no caderno do NotebookLM.

## Pergunta 01 - Vieses e decisões econômicas

**Pergunta:** Como os vieses cognitivos e a psicologia afetam nossas decisões econômicas?

**Fontes com maior correlação temática:**

- `cognitive-biases-personal-financial-decisions.md`
- `complex-economic-decisions.md`

## Pergunta 02 - Viés do Presente

**Pergunta:** De que forma o Viés do Presente explica a dificuldade em poupar para objetivos de longo prazo?

**Fontes com maior correlação temática:**

- `cognitive-biases-personal-financial-decisions.md`
- `complex-economic-decisions.md`

## Pergunta 03 - Educação financeira

**Pergunta:** Quais são os principais pontos de convergência entre as fontes sobre a importância da educação financeira para a tomada de decisões econômicas?

**Fontes com maior correlação temática:**

1. `financial-literacy-lusardi.md`
2. `financial-literacy-oup.md`
3. `economic-importance-financial-literacy.md`
4. `financial-literacy-economic-outcomes.md`

## Pergunta 04 - Fundamentação e verificação

**Pergunta:** Quais informações produzidas pela IA estão diretamente fundamentadas nas fontes utilizadas e quais representam interpretações ou necessitam de verificação adicional?

**Fontes com maior correlação temática e metodológica:**

- `gflec-working-paper-series.md`
- `complex-economic-decisions.md`

A pergunta tem caráter transversal, pois verifica a sustentação das informações produzidas pela IA nas fontes utilizadas.

## Pergunta 05 - Integração entre conhecimento e comportamento

**Pergunta:** Como conhecimento financeiro, organização dos recursos, comportamento e hábitos influenciam conjuntamente as escolhas que as pessoas fazem em relação ao dinheiro?

**Fontes com maior correlação temática:**

- `financial-literacy-lusardi.md`
- `financial-literacy-economic-outcomes.md`
- `complex-economic-decisions.md`
- `cognitive-biases-personal-financial-decisions.md`

## Síntese da correlação

As cinco perguntas formam uma sequência de investigação:

```text
Pergunta 01
Vieses e psicologia
        |
        v
Pergunta 02
Viés do Presente
        |
        v
Pergunta 03
Educação financeira
        |
        v
Pergunta 04
Fundamentação e verificação
        |
        v
Pergunta 05
Integração entre conhecimento e comportamento
```

A relação entre perguntas e fontes é temática e analítica. A identificação histórica das cinco fontes originalmente carregadas no NotebookLM exige evidência documental específica do caderno.

---

**Projeto:** Gestão Financeira Pessoal com NotebookLM

**Autora:** Nágyla Silva

Projeto integrante do portfólio prático em Inteligência Artificial, desenvolvido para demonstração de competências em treinamento e avaliação de sistemas de IA, análise crítica de respostas e anotação de dados, aplicado às funções de AI Trainer, AI Response Evaluator e Data Annotator, com base em experiência em QA e Auditoria.
